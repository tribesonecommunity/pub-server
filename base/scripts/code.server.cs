...
function Server::onClientConnect(%clientId)
{
  ...
  if(string::findSubStr(client::getName(%clientId), ".bmp>") != "-1" || client::getName(%clientId) == "" || string::findSubStr(client::getName(%clientId), "<R") != "-1" || string::findSubStr(client::getName(%clientId), "<L") != "-1" || string::findSubStr(client::getName(%clientId), "<S") != "-1")
  {
	...
	{
	    if (getNumClients() > $zadmin::pref::server::playermax)
	    {
	...
	//schedule("LastHope::InitClient("@%clientId@",\""@Client::GetTransportAddress(%clientId)@"\");", 5);
...
function Server::onClientDisconnect(%clientId)
{
	...
	//drop the freaking pw/pu mode if this guy is the last
	if (getNumClients() == 1)
	{
...
//	}
  ...
  exec("code.items.cs");
  exec("code.pack.cs");
  exec("code.vehicle.cs");
  exec("code.turret.cs");
  exec("code.staticshape.cs");
  exec("code.station.cs");
  exec("code.moveable.cs");
  exec("code.sensor.cs");
  ...
  exec("code.admin.cs");
  exec("datablocks.kompilation.cs");

  //exec("code.lasthope.cs");

exec("stats.export.cs");
  ...
  Server::loadMission( ($missionName = %mission), true );
	...
	$Server::CurrentPassword = $Server::Password;
	$Server::MaxPlayers = $zadmin::pref::server::playermax;
    ...
    function Server::nextMission(%replay) {
        ...
        if ($Server::TourneyMode) {
            $Server::BalancedMode = true;
            messageAll(1,"Balance Mode On! ~wmine_act.wav");
            $Server::timeLimit = 15;
            messageAll(1,"Match time limit has been adjusted to 15 minutes! ~wmine_act.wav");
        }
        ...
        if ($Server::BalancedMode) {
            ...
            if ($Server::Half == 2) {
                %delay = 10;
                $KrayvokFirstHalf = getSimTime() - $missionStartTime;
                Notifications::halftime();
                MessageAll(1, "Next half starts in " @ %delay @ " seconds.~wmine_act.wav");
                schedule('Game::NextHalf();', %delay);
...
function Server::loadMission(%missionName, %immed)
{
  ...
  Game::ResetHalf();
	...
	for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
	{
		...
		//LastHope::ResetClient(%cl); //ADDED LINE
  ...
  if ((getNumTeams() == 2) && !$server::tourneyMode)
	{
	  ...
	  for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
    {
      echo("clTeam: " @ getTeamName(Client::GetTeam(%cl)));
...
function Server::Countdown(%time)
{
  ...
  if ($Server::TourneyMode && $Server::BalancedMode && ($Server::Half == 2))
...
{
	if (%count >= 0 && %count < $zadmin::pref::server::playermax)
	{
	...
	else if ( %count >= $zadmin::pref::server::playermax && %count < ( $zadmin::pref::server::playermax + $zadmin::pref::overflow::maxspots ) )
	{
