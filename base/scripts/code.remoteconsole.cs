...
$RemoteConsole::MinAccessRequired = GetAdminLevel("gg");
...
function RemoteConsole::displayMenuServerToggles(%clientId)
{
	addLine("Enable RemoteConsole", "mce", !%clientId.remoteConsole && (%clientId.adminLevel >= $RemoteConsole::MinAccessRequired), %clientId);
	addLine("Disable RemoteConsole", "mce", %clientId.remoteConsole && (%clientId.adminLevel >= $RemoteConsole::MinAccessRequired), %clientId);
}

function RemoteConsole::processMenuServerToggleMenu(%clientId, %sel)
{
	if((%sel == "mce"))
  {
    %clientId.remoteConsole = !%clientId.remoteConsole;
    RemoteConsole::Update(%clientId);
  }
