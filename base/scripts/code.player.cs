...
function Player::onDamage(%this,%type,%value,%pos,%vec,%mom,%vertPos,%quadrant,%object)
{
	if (Player::isExposed(%this))
	{
		...
		//team variables
		%dmgTeam = Client::getTeam(%damagedClient);
		%shotTeam = Client::getTeam(%shooterClient);
		%sameTeam = (%dmgTeam == %shotTeam);

		// gay chain hit sound
		//if (%type == $BulletDamageType)
		//	Client::sendMessage(%shooterClient,0,"~wLeftMissionArea.wav");

		Player::applyImpulse(%this, %mom);
	
		if($teamplay && %damagedClient != %shooterClient && %sameTeam )
		{
			if (%shooterClient != -1)
			{
				%curTime = getSimTime();
		...
		if (!Player::isDead(%this))
		{
			...
			if (%value)
			{
				// adjust chain damage over time
				//if ( %type == $BulletDamageType ) {
				//	%rawTime = getIntegerTime( true );
				//	%chainDamagePercent = ( 5 * ( %rawTime - %this.chainDamageTime ) ) / 32.0;
				//	%this.chainDamageTime = %rawTime;
				//	
				//	if ( %%chainDamagePercent > 1 ) {
				//		%chainDamagePercent = 1;
				//	}
				//	
				//	%value = ( %value * %chainDamagePercent );
				//	echo("Doing "@( %chainDamagePercent )@"% damage");
				//}
