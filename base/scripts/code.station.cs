...
function InventoryStation::onResupply(%this,%InvShopList)
{
	...
	if (GameBase::isActive(%this))
	{
		...
		if (%player != -1 && %this.lastPlayer == %player)
		{
			...
			if (%this.target != %client)
			{
				...
				//active code
				if (%InvShopList == "RemoteInvList")
					zadmin::ActiveMessage::Single(%client, EnterStation, "RemoteInventory");
				else if (%InvShopList == "InvList")
					zadmin::ActiveMessage::Single(%client, EnterStation, "Inventory");
			}
	
			%player.waitThrowTime = getSimTime();
			
			schedule("InventoryStation::onResupply(" @ %this @ ",'" @ %InvShopList @ "');",0.5,%this);
	...
	{ 
		...
		//active code
		zadmin::ActiveMessage::Single(%this.target, ExitStation);
...
function VehicleStation::onEndSequence(%this,%thread)
{
	...
	{
		%client = Player::getClient(%player);
		zadmin::ActiveMessage::Single(%client, EnterStation, "Vehicle");
	}
}
...
function VehiclePad::onDeactivate(%this)
{
	...
	{
		%client = Player::getClient(%player);
		zadmin::ActiveMessage::Single(%client, ExitStation);
	}
}
