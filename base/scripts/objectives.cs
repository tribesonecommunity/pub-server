echo("****** ["@$zadmin::version@"/"@$zadmin::state@"] ******");
...
{
  if ($Server::TourneyMode && $Server::BalancedMode && ($Server::Half == 1))
  {
    // I think the below causes some kind of race condition?
...
function Flag::onCollision(%this, %object)
{
  ...
  if (!Player::ObstructionsBelow(%object, 2*$Game::Midair::Height))
    %midair = true;
  ...
  if(%flagTeam == %playerTeam)
  {
    ...
    if(!%this.atHome)
    {
      ...
      if (%midair)
        Announcer::MidairFlagReturn(Client::GetName(%playerClient), getTeamName(%flagTeam));
        //zadmin::ActiveMessage::All( MidairFlagReturn, Client::GetName(%playerClient), getTeamName(%flagTeam) );
    }
    else
    {
  ...
  else
  {
    ...
    if(%flagTeam != -1)
    {
      %passer = $Announcer::Passer[%playerTeam];
      if (%midair && %passer != -1 && %passer != %playerClient)
        Announcer::MidairPass(Client::GetName(%passer), Client::GetName(%playerClient), getTeamName(%flagTeam));
        //zadmin::ActiveMessage::All( MidairPass, Client::GetName(%passer), Client::GetName(%playerClient), getTeamName(%flagTeam) );
